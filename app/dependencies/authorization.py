...
def get_user_with_claims(current_user=Depends(get_user)) -> auth.UserRecord:
    user = auth.get_user(current_user.get('uid'))
    return user


def get_professional_user(user = Depends(get_user_with_claims)) -> auth.UserRecord:
    if not user.custom_claims.get('role', 0) == 1:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
