...
class SelfcareRepository(BaseRepository):
    ...
    async def find_selfcare_tip_by_id(self, *, tip_id: str):
        selfcare_tip = await self.get_entity(Entities.SELFCARE_TIPS).find_one({"_id": tip_id})
    ...
    async def search_selfcare(self, *, query: str, to: str, limit: int):
        ...
        result = []
        selfcare_results = await self.get_entity(Entities.SELFCARE_TIPS).aggregate(pipeline).to_list(length=limit)
