...
class Notification():
    ...
    def __init__(self, android: Optional[_notifee_utils.NotificationAndroid] = None, body: Optional[str] = None, data: Optional[dict[str, str]] = None, id: Optional[str] = None, ios=None, subtitle: Optional[str] = None, title: Optional[str] = None):
...
class NotificationEncoder(json.JSONEncoder):
    ...
    def encode_android(cls, android):
        ...
        result = {
            'channelId': _Validators.check_string(
                'NotificationAndroid.channel_id', android.channelId),
            'largeIcon': _Validators.check_string(
                'NotificationAndroid.largeIcon', android.largeIcon, non_empty=True),
            'color': _Validators.check_string('NotificationAndroid.color', android.color, non_empty=True),
            'importance': android.importance,
            'actions': [cls.encode_android_action(android_action) for android_action in android.actions],
            'smallIcon': _Validators.check_string(
                'NotificationAndroid.largeIcon', android.smallIcon, non_empty=True)
        }
    ...
    def encode_android_action(cls, action):
        """Encodes an ``AndroidAction`` instance into JSON."""
        if action is None:
            return None
        ...
        result = {
            'icon': action.icon,
            # TODO add class encoder https://notifee.app/react-native/reference/androidinput
            'input': action.input,
            # TODO add class encoder https://notifee.app/react-native/reference/notificationpressaction
            'pressAction': NotificationEncoder.encode_notification_press_action(action.press_action),
            'title': action.title
        }
    ...
    @classmethod
    def encode_notification_press_action(cls, press_action):
        """Encodes an ``NotificationPressAction`` instance into JSON."""
        if press_action is None:
            return None
        if not isinstance(press_action, _notifee_utils.NotificationPressAction):
            raise ValueError('AndroidAction.press_action must be an instance of '
                             'NotificationPressAction class.')
        result = {
            'id': _Validators.check_string(
                'NotificationPressAction.id', press_action.id, non_empty=True),
            'launchActivity': _Validators.check_string(
                'NotificationPressAction.launch_activity', press_action.launch_activity),
            'launchActivityFlags': _Validators.check_number_list('NotificationPressAction.launch_activity_flags', press_action.launch_activity_flags),
            'mainComponent':  _Validators.check_string(
                'NotificationPressAction.main_component', press_action.main_component)
        }
        result = cls.remove_null_values(result)
        return result
